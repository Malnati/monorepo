...
- A documentação oficial deste repositório reside **exclusivamente** em `docs/rup/`. É **estritamente proibido** adicionar arquivos ou diretórios de documentação fora dessa árvore sem solicitação explícita no escopo da tarefa.
- **Arquivos proibidos na raiz do repositório:** Não crie arquivos de documentação, relatórios, resumos, auditorias ou qualquer tipo de documento `.md` na raiz do repositório (exemplos de nomes proibidos: `AUDIT_*.md`, `REPORT_*.md`, `SUMMARY_*.md`, `ANALYSIS_*.md`, `REVIEW_*.md`, `TODO.md`, `NOTES.md`, etc.). **Toda documentação deve estar em `docs/rup/` seguindo a estrutura RUP.**
...
- Toda inclusão ou atualização em `docs/rup/` deve seguir o modelo RUP descrito em [`docs/rup/README.md`](docs/rup/README.md), registrando o artefato na fase correspondente e vinculando a mudança ao changelog da entrega.
...
- **Para agentes de IA:** Se você precisa criar documentação de auditoria, relatórios de análise, resumos ou qualquer tipo de documento técnico, identifique primeiro a fase RUP apropriada em `docs/rup/` (ex.: `docs/rup/06-governanca-tecnica-e-controle-de-qualidade/` para auditorias) e crie o arquivo lá, nunca na raiz do repositório.
- Para consultas sobre ferramentas, metodologias, frameworks ou pesquisas já avaliadas, priorize os artefatos vigentes em `docs/rup/` e o acervo de referências em `docs/rup/99-anexos/reference/`. Utilize esse diretório antes de recorrer a fontes externas e registre no changelog qualquer novo material incorporado.

## 📘 Wiki RUP — Panorama Geral (legado) e espelho ativo em `docs/rup/`
- A wiki legada permanece preservada em `.ref/docs/wiki/` como referência histórica completa da documentação RUP do SACIR.
- A derivação viva do repositório encontra-se em `docs/rup/`, mantendo um espelho estruturado da wiki com os mesmos domínios RUP, atualizado e apto a receber melhorias.
- Sempre que restaurar conteúdo a partir da wiki, preserve a rastreabilidade citando tanto o caminho legado (`.ref/docs/wiki/...`) quanto o destino correspondente em `docs/rup/`.

### Documentos por fase (hierarquia conforme `.ref/docs/wiki/` ↔ `docs/rup/`)
- `.ref/docs/wiki/00-visao/` e `docs/rup/00-visao/`
...
- `.ref/docs/wiki/01-arquitetura/` e `docs/rup/01-arquitetura/`
...
- `.ref/docs/wiki/02-design/` e `docs/rup/02-design/`
...
- `.ref/docs/wiki/02-planejamento/` e `docs/rup/02-planejamento/`
...
- `.ref/docs/wiki/03-implementacao/` e `docs/rup/03-implementacao/`
...
- `.ref/docs/wiki/04-testes-e-validacao/` e `docs/rup/04-testes-e-validacao/`
...
- `.ref/docs/wiki/05-entrega-e-implantacao/` e `docs/rup/05-entrega-e-implantacao/`
...
- `.ref/docs/wiki/06-governanca-tecnica-e-controle-de-qualidade/` e `docs/rup/06-governanca-tecnica-e-controle-de-qualidade/`
...
- `.ref/docs/wiki/06-ux-brand/` e `docs/rup/06-ux-brand/`
...
- `.ref/docs/wiki/07-contribuicao/` e `docs/rup/07-contribuicao/`
  - `README.md` — colaboração, commits e PRs.
  - Subdocumentos: `contribuindo.md`, `padroes-de-commit.md`, `template-de-pr.md`.
- `.ref/docs/wiki/99-anexos/` e `docs/rup/99-anexos/`
  - `README.md` — glossário e referências de apoio.
  - Subdocumentos: `glossario.md`, `referencias.md`.

### Acervos históricos e validação
- `.ref/docs/wiki/03-agentes-ia/` e `docs/rup/03-agentes-ia/` — histórico de agentes, pipelines e políticas para auditoria de IA.
- `.ref/docs/wiki/04-qualidade-testes/` e `docs/rup/04-qualidade-testes/` — documentação anterior de QA preservada para consulta.
- `.ref/docs/wiki/05-operacao-release/` e `docs/rup/05-operacao-release/` — registros legados de ambientes, publicação e versionamento.
- `.ref/docs/wiki/validation-report.md` e `docs/rup/validation-report.md` — certificação de atualização da wiki e rastreio de pendências.

### Onde registrar novos requisitos
- Utilize o índice RUP para escolher o diretório correspondente na árvore `docs/rup/`, referenciando sempre a origem legado em `.ref/docs/wiki/` quando houver.
- Requisitos não funcionais ou restrições técnicas devem ser mantidos em `docs/rup/01-arquitetura/` e espelhados conforme `requisitos-nao-funcionais.md` legado.
- Especificações funcionais, fluxos e contratos devem ser registrados em `docs/rup/02-design/`, alinhados aos artefatos correspondentes na wiki.
- Critérios e planos de teste derivados de novos requisitos precisam estar em `docs/rup/04-testes-e-validacao/`, mantendo vínculo com os cenários históricos.
- Requisitos ligados a entrega, governança ou UX devem atualizar simultaneamente `docs/rup/05-entrega-e-implantacao/`, `docs/rup/06-governanca-tecnica-e-controle-de-qualidade/` e `docs/rup/06-ux-brand/`, citando as seções legadas.

### Observações finais
- Nenhuma regra ou especificação deve ficar fora da árvore `docs/rup/`. Utilize `.ref/docs/wiki/` apenas como fonte de verdade para restauração e comparação.
- Registre no changelog toda decisão que racionalize divergências entre legado e derivação, preservando âncoras cruzadas.
- Consulte os acervos históricos apenas quando necessário para contexto, priorizando sempre os artefatos vigentes em `docs/rup/` para execução atual.


## 📘 RUP — Panorama Geral
- A pasta `docs/rup/` centraliza a documentação RUP da solução e de todos os subprojetos. Consulte [`docs/rup/README.md`](docs/rup/README.md) para visão geral das fases, convenções e índice atualizado.
...
- Conteúdos históricos permanecem acessíveis em `docs/rup/03-agentes-ia/`, `docs/rup/04-qualidade-testes/` e outras pastas de arquivo. Use-os como referência apenas quando indicado pelos documentos vigentes.
- O diretório `docs/rup/99-anexos/reference/` consolida guias operacionais, checklists e manuais de ferramentas adotadas. Consulte-o para confirmar parâmetros, fluxos e boas práticas antes de propor ajustes ou adotar novas abordagens.
...
### Requisitos gerais
- Cada subprojeto JavaScript ou TypeScript deve incluir `package.json`, TypeScript em modo `strict` e Vite como ferramenta padrão de bundling para interfaces web. Exceções precisam ser documentadas em [`docs/rup/03-implementacao/estrutura-de-projeto.md`](docs/rup/03-implementacao/estrutura-de-projeto.md).
...
- Preserve os diretórios estruturais descritos na arquitetura vigente. Alterações estruturais demandam atualização prévia da documentação em `docs/rup/` e registro em changelog.
- Valores de configuração devem seguir a cadeia `.env` → `docker-compose.yml` → serviço, conforme [`docs/rup/05-entrega-e-implantacao/ambientes-e-configuracoes.md`](docs/rup/05-entrega-e-implantacao/ambientes-e-configuracoes.md).

### Docker
- **Separação de responsabilidades:** Os arquivos `Dockerfile` deste projeto devem ser utilizados exclusivamente para instalar dependências e executar o processo da aplicação. Variáveis de ambiente, volumes, healthchecks, checagens e execuções de comandos devem ser definidos exclusivamente no `docker-compose.yml`.
...
- Não declare a chave `version` nos arquivos de compose e mantenha os templates `.env.example` alinhados às validações documentadas no acervo `docs/rup/`.
...
- Atualize [`docs/rup/03-implementacao/estrutura-de-projeto.md`](docs/rup/03-implementacao/estrutura-de-projeto.md) e o changelog correspondente sempre que novos alvos, serviços ou variáveis forem introduzidos.
...
## Monitoramento
 Sempre que arquivos forem removidos do repositório (por agentes automatizados ou humanos), é OBRIGATÓRIO:
 
 1. Revisar todos os arquivos do projeto para identificar referências, dependências, imports, scripts, configurações ou documentação que possam ter sido afetados pela exclusão.
 2. Remover ou corrigir código morto, referências órfãs, imports quebrados e dependências não utilizadas resultantes da exclusão.
 3. Registrar no changelog:
   - Lista dos arquivos removidos
   - Lista dos arquivos revisados/afetados
   - Descrição das verificações realizadas (referências, dependências, scripts, documentação)
   - Ações corretivas aplicadas
   - Referência cruzada à documentação RUP relevante (`docs/rup/`)
 4. Validar que todas as regras de rastreabilidade, documentação e governança técnica foram atendidas, conforme `AGENTS.md` e artefatos em `docs/rup/`.
 5. Utilizar o checklist abaixo antes de concluir a entrega:
 
 ### Checklist obrigatório para exclusão de arquivos
 - [ ] Todos os arquivos do projeto revisados para referências à exclusão
 - [ ] Código morto e imports órfãos removidos
 - [ ] Changelog detalhado e vinculado à documentação RUP
 - [ ] Conformidade com rastreabilidade e governança técnica
 - [ ] Auditoria cruzada com artefatos em `docs/rup/`
 
 Se qualquer item não for atendido, interrompa a entrega, corrija e registre a justificativa no changelog.
- Ajustes em portas, coletores ou dashboards devem ser documentados previamente nos artefatos de Entrega e Governança (`docs/rup/05-entrega-e-implantacao/` e `docs/rup/06-governanca-tecnica-e-controle-de-qualidade/`).
...
- Cada plano precisa conter, na ordem apresentada, seções tituladas para:
  ...
  8. Seleção dos checklists em `docs/rup/99-anexos/checklists/` aplicáveis ao contexto, referenciando-os no plano e destacando quais são de uso obrigatório independentemente do tema.
...
- Sempre que um plano for criado ou atualizado, deve existir um plano de auditoria irmão no mesmo diretório com o mesmo prefixo `YYYYMMDDHHMMSS-` e sufixo `-audit` que replique o padrão observado em `docs/rup/99-anexos/planos/*-audit.md`.
...
- **Terminologia obrigatória:** Use exclusivamente "APP" ou "plataforma APP" em toda interface do usuário, formulários, políticas LGPD, termos de uso e conteúdo frontend.
- **Reserva futura:** O termo "App" está reservado exclusivamente para a futura fase Fintech/Moeda e não deve aparecer em textos que impactam a experiência do usuário.
- **Contextos permitidos:** "App" pode ser mantido apenas em:
  - Comentários de código e cabeçalhos de arquivos para contexto técnico
  - Documentação arquitetural interna (`docs/rup/`)
  - Scripts SQL e migrações para rastreabilidade
  - Nomes de repositório e branches (por limitação técnica)
- **Substituições padrão:**
  - "App" → "APP" (referências diretas)
  - "plataforma App" → "plataforma APP" (contextos descritivos)
  - "do App" → "da APP" (adaptação gramatical)
  - "o App" → "a APP" (adaptação gramatical)
- **Validação obrigatória:** Antes de qualquer commit que altere textos frontend, execute `grep -r "App" landing/src/` para garantir zero ocorrências.
- **Requisito relacionado:** Esta política implementa `REQ-405` (Consistência de branding APP) conforme documentado em `docs/rup/02-planejamento/requisitos-spec.md`.

## Regras para integrações adicionais
- Novos serviços (por exemplo, gateways, APIs especializadas ou integrações serverless) só podem ser adicionados após documentação prévia em `docs/rup/01-arquitetura/` e `docs/rup/02-design/`. Atualmente não há subprojetos PostgREST ativos neste repositório.
- Ao homologar uma nova integração, registre as convenções técnicas na fase correspondente do acervo `docs/rup/` e atualize este documento na revisão subsequente.
...
### 🔹 Estrutura obrigatória de documentos `docs/rup/`

- **Escopo:** todos os arquivos dentro do diretório `docs/rup/` e de seus subdiretórios.
...
- **Criação de novos artefatos:** ao precisar de um novo documento em `docs/rup/`, crie simultaneamente o par `A.md`/`A-spec.md`, atualize os índices relevantes (`docs/rup/README*.md`) e registre o motivo no `CHANGELOG` e em `docs/rup/audit-history*.md`.
...
- **Validação automática:** antes de abrir PR, confirme que nenhum arquivo `docs/rup/**/` ficou sem par através de `find docs/rup -name "*.md"` e verifique se todo `*-spec.md` possui o guia correspondente. Diferentes nomes exigem correspondência literal (ex.: `visao-do-produto.md` ↔ `visao-do-produto-spec.md`).
...
- As regras completas de layout do painel lateral, tokens de identidade visual, acessibilidade e QA estão catalogadas em [`docs/rup/06-ux-brand/diretrizes-de-ux.md`](docs/rup/06-ux-brand/diretrizes-de-ux.md).
- Utilize as seções deste documento sobre cores (603010), tipografia (4x2), espaçamento (8pt Grid), simplicidade visual e UX Writing como checklist resumido, sempre confirmando os detalhes na documentação em `docs/rup/` antes de alterar componentes.
...
- **Responsável:** conforme definido em [`docs/rup/05-entrega-e-implantacao/empacotamento.md`](docs/rup/05-entrega-e-implantacao/empacotamento.md) ou artefato equivalente.
- **Processo:** upload manual via Chrome Web Store Developer Console seguindo o checklist publicado em `docs/rup/`.
...
- Consulte [`docs/rup/06-governanca-tecnica-e-controle-de-qualidade/revisoes-com-ia.md`](docs/rup/06-governanca-tecnica-e-controle-de-qualidade/revisoes-com-ia.md) para a lista de modelos aprovados, tokens autorizados e fluxos de auditoria.
...
- O catálogo de requisitos oficial está em `docs/rup/02-planejamento/especificacao-de-requisitos.md`; qualquer alteração funcional, técnica ou legal deve atualizar o documento e citar o ID `REQ-###` correspondente nos PRs e relatórios.
- A matriz de riscos mantida em `docs/rup/02-planejamento/riscos-e-mitigacoes-spec.md` precisa ser revisada sempre que novos riscos forem identificados ou mitigados; agents devem registrar decisões e apontar `RISK-###` nos metadados exportados.
- O guia de relatórios automatizados vive em `docs/rup/06-governanca-tecnica-e-controle-de-qualidade/auditoria-e-rastreabilidade.md#catalogo-de-relatorios-automatizados`; ajustes em pipelines ou geração de artefatos devem manter a tabela atualizada e sincronizar gatilhos com `revisoes-com-ia.md`.
- O plano da capacidade de diagnóstico colaborativo está em `docs/rup/02-planejamento/capacidade-diagnostico-colaborativo.md`; mantenha os requisitos `REQ-031` a `REQ-045` sincronizados com riscos e relatórios antes de alterar fluxos ou pipelines.
- Nenhum agente pode excluir ou mover esses arquivos sem confirmar que todas as referências em `docs/rup/` e nos diretórios `docs/reports/` foram ajustadas e auditadas pelo pipeline `audit.yml`.

## 🔹 Controle de ativos visuais da APP
- Adições ou substituições de logos devem seguir estritamente as orientações de aplicação descritas em `docs/rup/06-ux-brand/identidades-visuais-spec.md`.
- É proibido versionar imagens que contenham textos adicionais além da palavra **APP** (ex.: slogans, selos ou claims); mantenha esses arquivos fora do repositório até liberação formal do comitê de marca.
- Sempre documente, no PR, o tipo de fundo previsto para cada logo incluída, garantindo aderência aos cenários descritos na especificação.
...
## 🔹 Referências de Responsabilidade Técnica
**Responsável técnico:** consulte `docs/rup/06-governanca-tecnica-e-controle-de-qualidade/governanca-tecnica.md` para o quadro atualizado.
...
- **CRÍTICO**: Confirme existência de `docs/rup/` e conformidade com estrutura RUP obrigatória, verificando que não existe diretório `rup/` para documentação
- **CRÍTICO**: Valide que `.env.example` existe e está sincronizado com `docker-compose.yml`
- **CRÍTICO**: Verifique se todos os arquivos possuem cabeçalhos de caminho apropriados
- **OBRIGATÓRIO**: Execute `find . -name "*.md" -exec grep -L "^<!--.*\.md -->" {} +` para detectar arquivos sem cabeçalho
- **OBRIGATÓRIO**: Confirme que mudanças estruturais estão documentadas em changelog antes de prosseguir

## 🔹 Prevenção de Regressões de Conformidade
### Checklist de Validação Pré-Execução
- [ ] `Makefile` existe na raiz e possui targets obrigatórios (`build`, `start`, `stop`, `clean`)
- [ ] Diretório `docs/rup/` existe com estrutura RUP completa
- [ ] Arquivo `.env.example` configurado com todas as variáveis do `docker-compose.yml`
- [ ] Todos os arquivos `.md` possuem cabeçalhos de caminho (`<!-- path/file.md -->`)
- [ ] Estrutura de pares `A.md`/`A-spec.md` implementada em `docs/rup/`
- [ ] Referências a `docs/rup/` estão consistentes no AGENTS.md (nenhuma referência a `rup/` deve existir)
- [ ] Orientações de ferramentas e metodologias revisadas em `docs/rup/` e em `docs/rup/99-anexos/reference/`
- [ ] Nenhum diretório `rup/` foi criado incorretamente para documentação
...
### Validação de Estrutura RUP
```bash
# Comando para validar estrutura básica obrigatória
test -f Makefile && test -d docs/rup && test -f .env.example && ! test -d req && echo "✅ Estrutura básica OK" || echo "❌ FALHA: Estrutura incompleta ou diretório rup/ incorreto existe"
```
...
| Validação pré-execução implementada | ✅ Adicionado |
| Prevenção de regressões ativa | ✅ Configurado |
