...
async function waitForAPI(): Promise<boolean> {
  ...
  for (let i = 0; i < MAX_RETRIES; i++) {
    try {
      ...
      if (i < MAX_RETRIES - 1) {
        log(`Tentativa ${i + 1}/${MAX_RETRIES} - API ainda não disponível, aguardando...`);
...
async function testEndpoint(
  ...
  try {
    ...
    if (isValidStatus) {
      addResult(name, true);
      return true;
    } else if (response.status === 401 && !useAuth && expectedStatuses.includes(401)) {
      ...
      addResult(name, false, `Status esperado: ${expectedStatuses.join(' ou ')}, recebido: ${response.status}${errorMessage}`);
      return false;
    }
  } catch (err) {
...
async function testTipos() {
  ...
  await testEndpoint('GET', '/tipos', 'GET /tipos - Listar tipos', undefined, [200, 401]);
}
...
async function testUnidades() {
  ...
  await testEndpoint('GET', '/unidades', 'GET /unidades - Listar unidades', undefined, [200, 401]);
}
...
async function testFornecedores() {
  ...
  await testEndpoint('GET', '/fornecedores', 'GET /fornecedores - Listar fornecedores', undefined, [200, 401, 404]);
  
  // GET /fornecedores/:id - Buscar fornecedor por ID (usando ID 1 do seed)
  await testEndpoint('GET', '/fornecedores/1', 'GET /fornecedores/:id - Buscar fornecedor', undefined, [200, 401, 404]);
  
  // GET /fornecedores/:id/avatar - Buscar avatar (usando ID 1 do seed)
  await testEndpoint('GET', '/fornecedores/1/avatar', 'GET /fornecedores/:id/avatar - Buscar avatar', undefined, [200, 401, 404]);
}
...
async function testCompradores() {
  ...
  await testEndpoint('GET', '/compradores', 'GET /compradores - Listar compradores', undefined, [200, 401, 404]);
  
  // GET /compradores/:id - Buscar comprador por ID (usando ID 1 do seed)
  await testEndpoint('GET', '/compradores/1', 'GET /compradores/:id - Buscar comprador', undefined, [200, 401, 404]);
}
...
async function testOffers() {
  ...
  await testEndpoint('GET', '/offers', 'GET /offers - Listar offers', undefined, [200, 401]);
  
  // GET /offers?page=1&limit=10 - Listar com paginação
  await testEndpoint('GET', '/offers?page=1&limit=10', 'GET /offers - Listar com paginação', undefined, [200, 401]);
  
  // GET /offers/:id - Buscar offer por ID (usando ID 1 do seed)
  await testEndpoint('GET', '/offers/1', 'GET /offers/:id - Buscar offer', undefined, [200, 401]);
}
...
async function testLotes() {
  ...
  await testEndpoint('GET', '/lotes', 'GET /lotes - Listar lotes (legacy)', undefined, [200, 401]);
  
  // GET /lotes?page=1&limit=10 - Listar com paginação
  await testEndpoint('GET', '/lotes?page=1&limit=10', 'GET /lotes - Listar com paginação', undefined, [200, 401]);
  
  // GET /lotes/:id - Buscar lote por ID (usando ID 1 do seed)
  await testEndpoint('GET', '/lotes/1', 'GET /lotes/:id - Buscar lote', undefined, [200, 401]);
}
...
async function testFotos() {
  ...
  await testEndpoint('GET', '/fotos/1', 'GET /fotos/:id - Buscar foto', undefined, [200, 401, 404]);
}
...
async function testTransacoes() {
  ...
  await testEndpoint('GET', '/transacoes', 'GET /transacoes - Listar transações', undefined, [200, 401]);
  
  // GET /transacoes?page=1&limit=10 - Listar com paginação
  await testEndpoint('GET', '/transacoes?page=1&limit=10', 'GET /transacoes - Listar com paginação', undefined, [200, 401]);
}
...
async function testFormaPagamento() {
  ...
  await testEndpoint('GET', '/forma-pagamento', 'GET /forma-pagamento - Listar formas de pagamento', undefined, [200, 401, 404]);
}
...
async function runTests() {
  ...
  if (!apiReady) {
    error('API não está disponível após múltiplas tentativas. Abortando testes.');
    process.exit(1);
  }
...
// Executar testes
runTests().catch((err) => {
  error(`Erro fatal durante execução dos testes: ${err instanceof Error ? err.message : String(err)}`);
  process.exit(1);
});
