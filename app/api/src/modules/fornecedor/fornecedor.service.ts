...
export class FornecedorService {
  ...
  async findOne(id: number, userId: number | null): Promise<FornecedorEntity> {
    this.logger.log(`findOne - User: ${userId || 'anonymous'} - Fornecedor ID: ${id}`);
    const fornecedor = await this.fornecedorRepository.findOne({ where: { id } });
    if (!fornecedor) {
      this.logger.warn(`findOne - User: ${userId || 'anonymous'} - Fornecedor ID: ${id} not found`);
      throw new NotFoundException(`Fornecedor com ID ${id} não encontrado`);
    }
    this.logger.log(`findOne - User: ${userId || 'anonymous'} - Fornecedor ID: ${id} found`);
    return fornecedor;
  }

  async create(nome: string, email?: string, avatar?: Buffer | null): Promise<FornecedorEntity> {
    const fornecedor = this.fornecedorRepository.create({ nome, email, avatar: avatar || undefined });
    return await this.fornecedorRepository.save(fornecedor);
  }

  async findOrCreate(nome: string, email?: string, avatar?: Buffer | null): Promise<FornecedorEntity> {
    const existing = await this.fornecedorRepository.findOne({ where: { nome } });
    if (existing) {
