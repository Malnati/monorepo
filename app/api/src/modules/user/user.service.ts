...
export class UserService {
  ...
  async addFornecedor(userId: number, fornecedorId: number): Promise<void> {
    ...
    const fornecedorRepo =
      this.userRepository.manager.getRepository("tb_fornecedor");
    const fornecedor = await fornecedorRepo.findOne({
  ...
  async addComprador(userId: number, compradorId: number): Promise<void> {
    ...
    const compradorRepo =
      this.userRepository.manager.getRepository("tb_comprador");
    const comprador = await compradorRepo.findOne({
