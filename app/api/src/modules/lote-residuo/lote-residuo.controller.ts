...
import { Controller, Get, Post, Body, Param, Query, Patch, HttpCode, HttpStatus, UseGuards, Request, Logger } from '@nestjs/common';
...
export class LoteResiduoController {
  ...
  @Get('meus')
  @UseGuards(JwtAuthGuard)
  findMyLotes(@Request() req: any, @Query('page') page?: string, @Query('pageSize') pageSize?: string) {
    const userId = req.user.sub;
  ...
  updateLocation(@Param('id') id: string, @Body() updateDto: UpdateLocationDto, @Request() req: any) {
    const userId = req.user?.sub || req.user?.id || null;
    return this.loteResiduoService.updateLocation(parseInt(id, 10), updateDto, userId);
  }
}
