...
export class LoteResiduoService {
  ...
  private buildLocationLayers(lote: LoteResiduoEntity) {
    ...
    if (lote.neighborhood_name && lote.neighborhood_location_raw) {
      const [neighLat, neighLng] = lote.neighborhood_location_raw
        .split(",")
        .map(parseFloat);
      layers.neighborhood = {
        latitude: neighLat,
    ...
    if (lote.city_name && lote.city_location_raw) {
      const [cityLat, cityLng] = lote.city_location_raw
        .split(",")
        .map(parseFloat);
      layers.city = {
        latitude: cityLat,
  ...
  private buildApproxLocationLayers(lote: LoteResiduoEntity) {
    ...
    if (
      ...
      const [neighLat, neighLng] = lote.approx_neighborhood_location_raw
        .split(",")
        .map(parseFloat);
      layers.neighborhood = {
        latitude: neighLat,
    ...
    if (lote.approx_city_name && lote.approx_city_location_raw) {
      const [cityLat, cityLng] = lote.approx_city_location_raw
        .split(",")
        .map(parseFloat);
      layers.city = {
        latitude: cityLat,
  ...
  async create(createDto: CreateLoteResiduoDto, userId: number) {
    ...
    if (createDto.fotos && createDto.fotos.length > MAX_FOTOS_PER_LOTE) {
      throw new BadRequestException(
        `Máximo de ${MAX_FOTOS_PER_LOTE} fotos por lote`,
      );
    }
    ...
    const lat =
      ...
      createDto.localizacao?.latitude;
    const lng =
      locationData?.longitude ||
      createDto.address?.longitude ||
      createDto.localizacao?.longitude;
    ...
    const result = {
      ...
      address: fullLote.formatted_address
        ? {
            ...
            latitude: fullLote.localizacao
              ? parseFloat(fullLote.localizacao.split(",")[0])
              : 0,
            longitude: fullLote.localizacao
              ? parseFloat(fullLote.localizacao.split(",")[1])
              : 0,
  ...
  async findAll(query: SearchLotesDto, userId: number | null) {
    ...
    const queryBuilder = this.loteResiduoRepository
      ...
      .addSelect(
        "CASE WHEN lote.location_geog IS NOT NULL THEN ST_X(lote.location_geog::geometry)::text ELSE NULL END",
        "longitude",
      )
      .addSelect(
        "CASE WHEN lote.location_geog IS NOT NULL THEN ST_Y(lote.location_geog::geometry)::text ELSE NULL END",
        "latitude",
      )
    ...
    // Text search
    if (query.search) {
      queryBuilder.andWhere("lote.titulo ILIKE :search", {
        search: `%${query.search}%`,
      });
    }
    ...
    queryBuilder.orderBy("lote.created_at", "DESC").skip(skip).take(limit);
    ...
    const data = (lotes as LoteWithCoordinates[]).map((lote) => {
      ...
      return {
        ...
        unidade: lote.unidade
          ? { id: lote.unidade.id, nome: lote.unidade.nome }
          : null,
        ...
        foto_principal:
          lote.fotos && lote.fotos.length > 0
            ? {
                id: lote.fotos[0].id,
                url: `/app/api/fotos/${lote.fotos[0].id}`,
              }
            : null,
  ...
  async findOne(id: number, userId?: number) {
    ...
    const quantidade_disponivel =
      Number(lote.quantidade) - Number(lote.quantidade_vendida);
    ...
    if (userId && lote.fornecedor_id) {
      const userFornecedorCheck = await this.loteResiduoRepository
        ...
        .innerJoin(
          "tb_user_fornecedor",
          "uf",
          "uf.fornecedor_id = lote.fornecedor_id",
        )
        .where("lote.id = :id", { id })
  ...
  async updateLocation(
    ...
    const logger = new Logger(LoteResiduoService.name);
    logger.log(
      `updateLocation - User: ${userId || "anonymous"} - Lote ID: ${id}`,
    );
