...
export class TransacaoService {
  ...
  async create(createDto: CreateTransacaoDto, userId: number | null) {
    ...
    logger.log(
      `create - User: ${userId || "anonymous"} - Creating transacao for offer: ${createDto.offer_id}`,
    );
    ...
    if (offer.has_transacao) {
      logger.warn(
        `create - User: ${userId || "anonymous"} - Attempt to purchase already sold offer: ${createDto.offer_id}`,
      );
      throw new BadRequestException({
        code: ERROR_CODES.OFFER_ALREADY_SOLD,
        message:
          "Esta oferta já foi vendida e não está mais disponível para compra",
        details: {
    ...
    if (userId && offer.fornecedor?.id) {
      ...
      try {
        const isUserFornecedor = await queryRunner.manager
          ...
          .andWhere("uf.fornecedor_id = :fornecedorId", {
            fornecedorId: offer.fornecedor.id,
          })
          .getCount();

        if (isUserFornecedor > 0) {
          logger.warn(
            `create - User: ${userId} - Attempt to purchase own offer: ${createDto.offer_id}`,
          );
          throw new BadRequestException({
    ...
    try {
      ...
      if (existingTransacao > 0) {
        logger.warn(
          `create - User: ${userId || "anonymous"} - Race condition detected for offer: ${createDto.offer_id}`,
        );
        throw new BadRequestException({
          code: ERROR_CODES.LOT_ALREADY_SOLD,
          message:
            "Esta oferta já foi vendida enquanto você estava processando a compra",
          details: {
            offer_id: createDto.offer_id,
          },
        });
      ...
      const updateResult = await queryRunner.manager
        ...
        .set({
          quantidade_vendida: () =>
            `quantidade_vendida + ${createDto.quantidade}`,
        })
        .where("id = :id", { id: createDto.offer_id })
        .andWhere("quantidade - quantidade_vendida >= :quantidade", {
      ...
      if (updateResult.affected === 0) {
        logger.warn(
          `create - User: ${userId || "anonymous"} - Insufficient quantity for offer: ${createDto.offer_id}`,
        );
        throw new BadRequestException({
          code: ERROR_CODES.INSUFFICIENT_QUANTITY,
      ...
      logger.log(
        `create - User: ${userId || "anonymous"} - Transacao created successfully with ID: ${savedTransacao.id}`,
      );
      ...
      this.sendTransactionEmails(transacaoCompleta, logger).catch((error) => {
        logger.error(
          `Failed to send transaction emails for ID ${savedTransacao.id}`,
          error,
        );
      });
  ...
  async findAllByUserId(userId: number | null) {
    ...
    const transacoes = await this.transacaoRepository
      ...
      .leftJoin(
        "tb_user_fornecedor",
        "uf",
        "uf.fornecedor_id = transacao.fornecedor_id",
      )
      .leftJoin(
        "tb_user_comprador",
        "uc",
        "uc.comprador_id = transacao.comprador_id",
      )
      .select([
        "transacao.id",
    ...
    return transacoes.map((transacao) => {
      const precoTotal =
        Number(transacao.offer.preco) * Number(transacao.quantidade);
      ...
      // Camada de bairro
      if (
        transacao.offer.neighborhood_name &&
        transacao.offer.neighborhood_location_raw
      ) {
        const [neighLat, neighLng] = transacao.offer.neighborhood_location_raw
          .split(",")
          .map(parseFloat);
      ...
      if (transacao.offer.city_name && transacao.offer.city_location_raw) {
        const [cityLat, cityLng] = transacao.offer.city_location_raw
          .split(",")
          .map(parseFloat);
        locationLayers.city = {
          latitude: cityLat,
      ...
      const finalLocationLayers =
        Object.keys(locationLayers).length > 0 ? locationLayers : null;
  ...
  async findOne(id: number, userId: number | null) {
    const logger = new Logger(TransacaoService.name);
    logger.log(
      `findOne - User: ${userId || "anonymous"} - Transacao ID: ${id}`,
    );
    ...
    logger.log(
      `findOne - User: ${userId || "anonymous"} - Transacao ID: ${id} found`,
    );
    return result;
  }
    ...
    try {
      ...
      if (!fornecedorEmail && transacao.fornecedor?.id) {
        ...
        const users = await this.dataSource
          ...
          .where("uf.fornecedor_id = :fornecedorId", {
            fornecedorId: transacao.fornecedor.id,
          })
          .limit(1)
          .getRawOne();
      ...
      if (!compradorEmail && transacao.comprador?.id) {
        ...
        const users = await this.dataSource
          ...
          .where("uc.comprador_id = :compradorId", {
            compradorId: transacao.comprador.id,
          })
          .limit(1)
          .getRawOne();
      ...
      if (fornecedorEmail) {
        emailPromises.push(
          this.gmailService.sendTransactionSuccessEmail(
            emailPayload,
            "fornecedor",
          ),
        );
      ...
      results.forEach((result, index) => {
        const recipient =
          index === 0 && fornecedorEmail ? "fornecedor" : "comprador";
