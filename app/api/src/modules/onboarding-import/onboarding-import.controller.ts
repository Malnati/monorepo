...
import { OnboardingImportService, BatchResponse } from './onboarding-import.service';
...
export class OnboardingImportController {
  ...
  constructor(private readonly onboardingImportService: OnboardingImportService) {}

  @Post('batch')
  async processBatch(@Body() batchDto: BatchOnboardingDto): Promise<BatchResponse> {
    this.logger.log(`Received batch from ${batchDto.source} with ${batchDto.users.length} users`);
