...
export class OnboardingService {
  ...
  async register(
    ...
    if (existingUser) {
      ...
      if (existingUser.status_ativacao === "pendente") {
        throw new ConflictException(
          "E-mail já cadastrado. Verifique sua caixa de entrada ou solicite reenvio",
        );
      }
    }
    ...
    this.logger.log(
      `Usuário aprovado pela IA: ${dto.email} (confidence: ${eligibilityCheck.confidence})`,
    );
    ...
    const emailSent = await this.mailingService.sendActivationEmail(
      dto.email,
      token,
      dto.nome,
    );
    ...
    if (!emailSent) {
      ...
      await this.registerAudit(
        ...
        userAgent,
        false,
        "Falha ao enviar e-mail de ativação",
      );
    } else {
      await this.registerActivationLog(
        user.id,
  ...
  async activate(
    ...
    if (user.status_ativacao === "ativado") {
      await this.registerAudit(
        ...
        userAgent,
        false,
        "Tentativa de reativação de conta já ativada",
      );
    ...
    if (user.token_expires_at && new Date() > user.token_expires_at) {
      ...
      throw new UnauthorizedException(
        "Token expirado. Solicite reenvio do e-mail de ativação.",
      );
    }
  ...
  async resend(
    ...
    if (user.status_ativacao === "ativado") {
      await this.registerAudit(
        ...
        userAgent,
        false,
        "Tentativa de reenvio para conta já ativada",
      );
      throw new ConflictException("Conta já ativada");
    }
    ...
    if (resendCount >= MAX_RESEND_ATTEMPTS_PER_DAY) {
      await this.registerAudit(
        ...
        userAgent,
        false,
        "Limite de reenvios excedido",
      );
      throw new BadRequestException(
        "Limite de reenvios excedido. Tente novamente em 24 horas.",
      );
    }
    ...
    if (!emailSent) {
      this.logger.error(
        `Falha ao reenviar e-mail de ativação para ${dto.email}`,
      );
      await this.registerAudit(
        user.id,
        "reenvio",
