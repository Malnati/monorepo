...
import { CSV_FILE_PATTERN, LOCK_FILE_SUFFIX, REPORT_FILE_SUFFIX } from '../../constants';
...
export class FilesystemService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.inboxDir = this.configService.get<string>('job.inbox') || '/var/data/inbox';
    this.processedDir = this.configService.get<string>('job.processed') || '/var/data/processed';
    this.failedDir = this.configService.get<string>('job.failed') || '/var/data/failed';
  }
  ...
  async findNewCsvFiles(): Promise<string[]> {
    try {
      ...
      return files.filter(file => 
        CSV_FILE_PATTERN.test(file) && 
        !file.endsWith(LOCK_FILE_SUFFIX)
      );
    } catch (error) {
  ...
  async releaseLock(filename: string): Promise<void> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.warn(`Failed to release lock for ${filename}: ${errorMessage}`);
    }
  }
