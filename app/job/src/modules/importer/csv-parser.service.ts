...
import { CsvRow, NormalizedUser, ValidationError } from '../../types/csv-row.interface';
import {
  CSV_ENCODING,
  CPF_LENGTH,
  PHONE_E164_PREFIX,
...
export class CsvParserService {
  ...
  async parseCsv(content: string): Promise<{ rows: CsvRow[]; errors: ValidationError[] }> {
    const rows: CsvRow[] = [];
  ...
  normalizeUser(row: CsvRow, lineNumber: number): { user?: NormalizedUser; errors: ValidationError[] } {
    const errors: ValidationError[] = [];
