...
export default function OnboardingActivationPage() {
  ...
  const handleActivation = async (emailParam: string, token: string) => {
    try {
      ...
      if (
        ...
        errorMessage.includes("já ativada") ||
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "Erro ao reenviar e-mail";
      showToast({
        variant: "error",
      ...
      case "success":
        return (
          <div className="w-16 h-16 rounded-full bg-green-100 dark:bg-green-900/20 flex items-center justify-center">
            <span
              className="material-symbols-rounded text-sm text-primary dark:text-green-400"
              style={{ fontSize: "3rem" }}
            >
              check_circle
            </span>
      ...
      case "invalid":
        return (
          <div className="w-16 h-16 rounded-full bg-red-100 dark:bg-red-900/20 flex items-center justify-center">
            <span
              className="material-symbols-rounded text-sm text-red-700 dark:text-red-400"
              style={{ fontSize: "3rem" }}
            >
              error
            </span>
  ...
  return (
    <div className="min-h-screen bg-surface dark:bg-dark-surface flex flex-col items-center justify-center p-4">
      {/* Logo */}
      <div className="mb-8">
        <img src={LOGO_PATH} alt="APP Logo" className="h-12 w-auto" />
      </div>

      {/* Card principal */}
      <div className="w-full max-w-md bg-white dark:bg-dark-card rounded-lg shadow-lg p-8 space-y-6">
        {/* Ícone de estado */}
        <div className="flex justify-center">{getStateIcon()}</div>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <label
                ...
                className="block text-sm font-medium text-dark-gray dark:text-gray-200"
              >
                E-mail
              </label>
              <input
      ...
      <div className="mt-8 max-w-md text-center">
        <p className="text-xs text-medium-gray dark:text-gray-500">
          Ao ativar sua conta, você concorda com nossos Termos de Uso e Política
          de Privacidade. Seus dados são protegidos conforme a LGPD.
        </p>
      </div>
    </div>
  );
