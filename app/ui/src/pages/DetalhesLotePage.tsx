  ...
  const handleComprar = async () => {
    ...
    try {
      ...
      const quantidadeDisponivel =
        Number(offer.quantidade) - Number(offer.quantidade_vendida || 0);
      ...
      if (!transacao || !transacao.id) {
        throw new Error(
          "Resposta da API inválida: ID da transação não encontrado",
        );
      }
      ...
      if (
        errorCode === "OWN_LOT_PURCHASE" ||
        errorCode === "OWN_OFFER_PURCHASE"
      ) {
        friendlyMessage =
          "Você não pode comprar seu próprio produto. Este produto foi cadastrado por você como fornecedor.";
      } else if (
        errorCode === "LOT_ALREADY_SOLD" ||
        errorCode === "OFFER_ALREADY_SOLD"
      ) {
        friendlyMessage =
          "Este produto já foi vendido para outro comprador e não está mais disponível.";
      } else if (errorCode === "INSUFFICIENT_QUANTITY") {
        friendlyMessage =
          "A quantidade solicitada não está mais disponível. Tente novamente com uma quantidade menor.";
      }
  ...
  return (
    <div className="font-display bg-background-light dark:bg-background-dark min-h-screen">
      <header className="sticky top-0 z-10 flex items-center justify-between bg-background-light/80 p-4 pb-2 backdrop-blur-sm dark:bg-background-dark/80">
        <button
          onClick={() => navigate(-1)}
          className="flex h-12 w-12 shrink-0 items-center justify-start text-text-light-primary dark:text-text-dark-primary"
        >
          <ICON_MAP.back className="h-6 w-6" aria-hidden="true" />
        </button>
        <h1 className="flex-1 text-center text-sm font-bold leading-tight tracking-[-0.015em] text-text-light-primary dark:text-text-dark-primary">
          {title}
        </h1>
        <TextButton
          onClick={logout}
      ...
      <main className="flex-grow pb-28">
          ...
          <div className="flex overflow-y-auto [-ms-scrollbar-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <div className="flex items-stretch gap-3 p-4">
              {offer.fotos.map((foto) => (
                <div
                  key={foto.id}
                  className="flex h-full min-w-[80vw] flex-1 flex-col gap-4 rounded-xl"
                >
        ...
        {(() => {
          ...
          const suggestedLocation =
            offer.approxLocationLayers?.neighborhood ||
            offer.approxLocationLayers?.city;
          ...
          return (
            <div className="px-4 py-4">
              <h3 className="mb-4 text-sm font-bold text-text-light-primary dark:text-text-dark-primary">
                Localização
              </h3>
              {hasLocation && (
                <StaticMap latitude={mapLat} longitude={mapLng} label={title} />
              )}
              <div className="mt-4">
        ...
        {offer.payment_methods &&
          offer.payment_methods.filter((forma) =>
            FORMAS_PAGAMENTO_PERMITIDAS.includes(forma.nome),
          ).length > 0 && (
            <div className="px-4 py-4">
              <h3 className="mb-4 text-sm font-bold text-text-light-primary dark:text-text-dark-primary">
                Formas de pagamento aceitas
          ...
          <FakePaymentOptions
            ...
            onConfirm={() => {
              showToast({
                ...
                description:
                  "Este é um fluxo simulado — nenhum valor foi cobrado",
              });
            }}
          />
        </div>
