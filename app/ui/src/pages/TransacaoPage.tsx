...
function AuthenticatedAvatar({
  ...
  return avatarUrl ? (
    <img
      ...
      alt={alt}
      className={
        className || "aspect-square rounded-full h-10 w-10 object-cover"
      }
      onError={(e) => {
    ...
    <div
      className={
        className ||
        "aspect-square rounded-full h-10 w-10 object-cover bg-gray-300"
      }
    ></div>
  );
}
    ...
    try {
      ...
      if (!hasShownEmailNotification.current) {
        ...
        showToast({
          ...
          description:
            "Os dados da transação foram encaminhados para o seu e-mail.",
          duration: 5000,
        });
      }
    } catch (error) {
    ...
    <div className="bg-background-light dark:bg-background-dark">
      <div className="relative flex min-h-screen w-full flex-col font-display group/design-root overflow-x-hidden">
        <header className="sticky top-0 z-10 flex items-center bg-background-light dark:bg-background-dark p-4 pb-2 justify-between">
          ...
          <h1 className="text-text-light-primary dark:text-text-dark-primary text-sm font-bold leading-tight tracking-[-0.015em] flex-1 text-center">
            Transação concluída
          </h1>
          <button
            onClick={logout}
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-3">
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                  <div className="flex items-center gap-2 sm:gap-4 flex-1 min-w-0">
                    ...
                    />
                    <p className="text-text-light-primary dark:text-text-dark-primary text-sm font-normal leading-normal flex-1 truncate min-w-0">
                  ...
                  <div className="flex flex-col gap-2 sm:gap-3">
                    <div className="flex items-center gap-2 sm:gap-4 flex-wrap">
                      {transacao.fornecedor.whatsapp && (
                        <a
                          href={getWhatsAppLink(
                            transacao.fornecedor.whatsapp,
                            `Olá ${transacao.fornecedor.nome}, gostaria de entrar em contato sobre a transação de ${transacao.lote_residuo.nome}.`,
                          )}
                          target="_blank"
                      ...
                      <p className="text-text-light-secondary dark:text-text-dark-secondary text-xs sm:text-sm font-normal leading-normal shrink-0">
                        Fornecedor
                      </p>
                    </div>
                    {/* Exibir endereço real do fornecedor */}
                    {(transacao.lote_residuo.formatted_address ||
                      realAddress) && (
                            ...
                            {transacao.lote_residuo.locationLayers.real && (
                              <div className="flex flex-col gap-2">
                                <button
                                  ...
                                  aria-label={
                                    isMapExpanded ? "Recolher mapa" : "Ver mapa"
                                  }
                                >
                                  {isMapExpanded ? (
                                    <>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                ...
                <div className="shrink-0">
                  <p className="text-text-light-secondary dark:text-text-dark-secondary text-xs sm:text-sm font-normal leading-normal">
                    Comprador
                  </p>
                </div>
              </div>
