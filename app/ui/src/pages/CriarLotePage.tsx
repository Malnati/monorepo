...
export default function CriarLotePage() {
  ...
  console.log(
    "[CriarLotePage] Contexto antes de useNavigate:",
    beforeUseNavigateContext,
  );
  ...
  console.log(
    "[CriarLotePage] formatCurrencyValue disponível?",
    typeof formatCurrencyValue,
  );

  const formatCurrencyLocal = useCallback((value: number | string): string => {
    console.log("[CriarLotePage] formatCurrencyLocal chamada", {
      value,
      formatCurrencyValueType: typeof formatCurrencyValue,
    });
    const numValue =
      typeof value === "string"
  ...
  const isLargeError = useCallback((errorMessage: string): boolean => {
    ...
    return (
      largeErrorIndicators.some((indicator) =>
  ...
  const extractFieldErrors = useCallback(
    (errorMessage: string): Record<string, string> => {
      ...
      if (
        ...
        errorMessage.toLowerCase().includes("descricao")
      ) {
        errors.descricao = "Verifique a descrição";
      }
      if (
        errorMessage.toLowerCase().includes("preço") ||
        errorMessage.toLowerCase().includes("preco")
      ) {
      ...
      if (
        errorMessage.toLowerCase().includes("localização") ||
        errorMessage.toLowerCase().includes("localizacao")
      ) {
  ...
  const loadTiposUnidades = useCallback(async () => {
    try {
      ...
      console.log(
        "[CriarLotePage] Todas as formas de pagamento recebidas:",
        todasFormasPagamento.map((f) => ({
          id: f.id,
          nome: f.nome,
          ativo: f.ativo,
        })),
      ...
      console.log(
        "[CriarLotePage] Formas permitidas normalizadas:",
        Array.from(permitidasNormalizadas),
      );

      const formasFiltradas = todasFormasPagamento.filter((forma) => {
        const nomeNormalizado = forma.nome.toLowerCase().trim();
        console.log("[CriarLotePage] Verificando forma:", {
          nome: forma.nome,
        ...
        for (const permitida of permitidasNormalizadas) {
          if (
            nomeNormalizado.includes(permitida) ||
            permitida.includes(nomeNormalizado)
          ) {
            console.log(
              "[CriarLotePage] ✓ Forma permitida (parcial):",
              forma.nome,
              "match com",
              permitida,
            );
        ...
        console.log(
          "[CriarLotePage] ✗ Forma de pagamento filtrada:",
          forma.nome,
        );
        return false;
      });
      ...
      console.log(
        "[CriarLotePage] Formas de pagamento filtradas:",
        formasFiltradas.map((f) => ({ id: f.id, nome: f.nome })),
      );
      setFormasPagamento(formasFiltradas);
    } catch (error) {
      console.error(
        "Erro ao carregar tipos/unidades/formas de pagamento:",
        error,
      );
    }
  }, []);
  ...
  const calculateSuggestedLocation = useCallback(
    (centerLat: number, centerLng: number, maxRadiusKm: number = 15) => {
      ...
      const newLng =
        centerLng + (distanceInDegrees * Math.sin(angle)) / latAdjustment;
    ...
    async (
      ...
      return new Promise((resolve, reject) => {
        ...
        const timeoutId = setTimeout(() => {
          console.warn(
            "Timeout na busca de ponto de referência, usando fallback",
          );
          resolve({
            formattedAddress: `${latitude.toFixed(6)}, ${longitude.toFixed(6)}`,
            placeId: "",
              ...
              try {
                ...
                if (
                  ...
                  const validResults = results.filter(
                    (place) => place.geometry?.location && place.place_id,
                  );
                  ...
                  if (validResults.length > 0) {
                    ...
                    safeResolve({
                      formattedAddress:
                        nearestPlace.vicinity ||
                        nearestPlace.formatted_address ||
                        "",
                  ...
                  (geocodeResults, geocodeStatus) => {
                    try {
                      if (
                        ...
                        const resultLat =
                          typeof resultLocation.lat === "function"
                            ? resultLocation.lat()
                            : resultLocation.lat;
                        const resultLng =
                          typeof resultLocation.lng === "function"
                            ? resultLocation.lng()
                            : resultLocation.lng;
                        ...
                        safeResolve({
                          ...
                          longitude: resultLng,
                          name:
                            result.formatted_address || "Localização próxima",
                        });
                      } else {
                        // Fallback: retornar a própria localização sugerida
                        console.warn(
                          "Geocoding falhou, usando localização sugerida como fallback",
                        );
                        safeResolve({
                          formattedAddress: `${latitude.toFixed(6)}, ${longitude.toFixed(6)}`,
                          placeId: "",
                ...
                console.error(
                  "Erro no callback do nearbySearch:",
                  callbackError,
                );
    ...
    try {
      ...
      if (error.response?.data) {
        ...
        if (Array.isArray(errorData.message)) {
          ...
          if (
            errorData.message.includes("fornecedor associado") ||
            errorData.message.includes("Faça login novamente")
          ) {
            console.log(
              "[CriarLotePage] Chamando showDialog (sessão expirada)",
              {
                showDialogType: typeof showDialog,
                showDialogValue: showDialog,
                navigateType: typeof navigate,
            ...
            showDialog({
              ...
              primaryAction: {
                ...
                onClick: () => {
                  console.log(
                    "[CriarLotePage] onClick do dialog - chamando navigate(/login)",
                    {
                      navigateType: typeof navigate,
                      navigateValue: navigate,
                    },
      ...
      if (isLargeError(errorMessage)) {
        ...
        if (
          errorMessage.includes("Publicação bloqueada") &&
          errorMessage.includes("campo limpo") &&
          errorMessage.includes("dados sensíveis")
        ) {
          displayMessage =
            "Revisão por Inteligência Artificial: Dados sensíveis encontrados no formulário. Por favor, descreva apenas sobre o material e como coletar.";
        }
        showDialog({
    ...
    <div className="font-display bg-background-light dark:bg-background-dark min-h-screen">
      <header className="sticky top-0 z-10 flex items-center bg-background-light dark:bg-background-dark p-4 pb-2 justify-between border-b border-background-light dark:border-background-dark">
        <button
          onClick={() => {
            console.log(
              "[CriarLotePage] Botão voltar clicado - chamando navigate(-1)",
              {
                navigateType: typeof navigate,
                navigateValue: navigate,
              },
          ...
          className="text-text-light-primary dark:text-text-dark-primary flex size-10 items-center justify-center rounded-full hover:bg-chip-light/40 dark:hover:bg-card-dark/60 transition-colors"
          aria-label="Voltar"
        >
          <ICON_MAP.back className="h-6 w-6" aria-hidden="true" />
        </button>
        <h1 className="text-text-light-primary dark:text-text-dark-primary text-sm font-bold leading-tight tracking-tight flex-1 text-center">
          Publicação
        </h1>
        <TextButton
          onClick={logout}
      ...
      <main className="flex-1 pb-28">
        <form onSubmit={handleSubmit} className="px-4 pt-6 space-y-4">
          ...
          <section>
            <h2 className="text-text-light-primary dark:text-text-dark-primary text-sm font-bold leading-tight tracking-tight pb-2">
            ...
            <div className="py-3">
              <div className="flex flex-col">
                ...
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-700 dark:text-gray-300 text-sm font-normal pointer-events-none z-10">
                    R$
                  </span>
                  <FilledTextField
                    label=""
          ...
          <section className="pt-4">
            ...
            <div className="py-3">
              ...
              >
                ...
                />
                <ICON_MAP.camera
                  className="h-12 w-12 text-text-light-secondary dark:text-text-dark-secondary mb-2"
                  aria-hidden="true"
                />
          ...
          <section className="pt-4 pb-24">
            ...
            <p className="text-sm text-text-light-secondary dark:text-text-dark-secondary pb-3">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 gap-3">
                {formasPagamento.map((forma) => {
                  const isSelected =
                    formData.payment_method_ids?.includes(forma.id) || false;
                  ...
                  if (forma.nome.toLowerCase().includes("pix")) {
                    ...
                    forma.nome.toLowerCase().includes("cartão") ||
          ...
          <footer className="fixed bottom-0 left-0 right-0 z-10 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-sm p-4 border-t border-background-light dark:border-background-dark">
            <ContainedButton
              ...
              icon={
                loading ? undefined : (
                  <ICON_MAP.create className="h-5 w-5" aria-hidden="true" />
                )
              }
            >
