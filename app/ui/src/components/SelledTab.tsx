...
const EMPTY_DESCRIPTION = 'Você ainda não possui lotes vendidos.';
...
export default function SelledTab() {
  const [lotesVendidos, setLotesVendidos] = useState<LoteResiduo[]>([]);
  ...
  const loadMeusLotes = async () => {
    try {
      ...
      setLotesVendidos(response.data.lotesVendidos || []);
    } catch (err) {
  ...
  const isEmpty = !loading && lotesVendidos.length === 0;
  ...
  return (
    <>
      ...
      {!loading && !error && lotesVendidos.length > 0 && (
