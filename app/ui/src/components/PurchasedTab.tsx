...
export default function PurchasedTab() {
  const [offersComprados, setOffersComprados] = useState<Offer[]>([]);
  ...
  const isEmpty = !loading && offersComprados.length === 0;
  ...
  return (
    <>
      ...
      {!loading && !error && offersComprados.length > 0 && (
        <div className="flex flex-col gap-4 pb-6 pt-4" role="list">
          {offersComprados.map((offer) => (
